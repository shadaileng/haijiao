...
> | 文档状态 | 🏁 已完成 |
> | 最后更新 | 2026-07-12 |
> | 对应功能 | 登录页面、登录 API、Sign 算法（已实现）、Store 状态管理、路由守卫 |
> | 实施步骤 | 10/10 已完成 |
>
> **变更历史**
>
> | 日期 | 版本 | 说明 |
> |------|:----:|------|
> | 2026-07-12 | v1.4.0 | 按方案重构：凭证保存集中到 user store，login() 仅负责 API 请求 |
> | 2026-07-12 | v1.3.0 | 根据实际代码重新分析，更新 login() 职责、user store 结构、交互流程 |
> | 2026-07-12 | v1.2.0 | 重构登录/退出逻辑，集中到 user store 中，符合文档方案架构 |
> | 2026-07-10 | v1.1.0 | 实施 Sign 算法（MD5(Username + Password + UserAgent)） |
> | 2026-07-10 | v1.0.0 | 初版，登录功能完整实施方案 |

> **关联文档**：[02-功能新增与改善方案.md](./02-功能新增与改善方案.md) §6 验证码交互 · §11 注册/找回密码
...
| `src/stores/user.ts` | 修改 | 新增 `loginFromApi()`、`logout()` |
...
| `src/types/index.ts` | 修改 | 新增 `LoginParams`、`LoginResponse` 类型 |
...
**职责说明**：`login()` 函数仅负责 API 请求和数据解密，凭证保存（uid/token）由 `user store` 的 `loginFromApi()` 方法负责。
...
**职责说明**：
- `loginFromApi(data)`：登录成功后调用，负责保存凭证（uid/token）到 settings store，并设置当前用户信息
- `logout()`：退出登录，负责清除 settings store 中的凭证，并清空当前用户信息
...
2. 点击"登录" → 显示 loading
3. 调用 `login()` API
4. 成功 → 调用 `userStore.loginFromApi(data)` → 跳转首页
5. 失败 → Toast 显示错误信息
...
**说明**：`LoginResponse.user` 复用 `UserCurrent` 接口，包含用户完整信息（id、nickname、avatar、email 等）。
