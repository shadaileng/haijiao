...
| 文件 | 操作 | 说明 |
|------|------|------|
| `src/api/request.ts` | 修改 | 新增 `login()` 函数（含凭证保存） |
| `src/stores/user.ts` | 修改 | 新增 `fetchCurrent()` 获取当前用户 |
...
**职责说明**：`login()` 函数负责 API 请求、数据解密和凭证保存（uid/token 到 settings store）。
...
**职责说明**：
- `fetchCurrent()`：获取当前用户信息并设置到 store
- 凭证保存（uid/token）由 `login()` 函数直接调用 `settings store` 的 `setCredentials()` 方法负责
- 退出登录由 `settings store` 的 `logout()` 方法负责
...
1. 用户输入邮箱/用户名和密码
2. 点击"登录" → 显示 loading
3. 调用 `login()` API（内部保存凭证）
4. 成功 → 调用 `userStore.fetchCurrent()` 获取用户信息 → 跳转首页
5. 失败 → Toast 显示错误信息
